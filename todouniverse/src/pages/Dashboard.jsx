...
console.log(isOpen ? "oi" :"nao oi")
return (
    
    <div className={`w-screen h-screen overflow-y-auto bg-night flex flex-1/2 flex-col`}>
        <NavBar />
        <div className={`flex-1 p-6 `}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="bg-blackOlive p-4 rounded-lg shadow-md border border-amber-400">
                    ...
                    <p className="text-3xl font-bold text-jonquil mt-2">{(tasks.filter((task) => task.status == "Pendente")).length}</p>
                </div>
                <div className="bg-blackOlive p-4 rounded-lg shadow-md border border-amber-400">
                    <h3 className="text-white font-medium">Projetos Ativos</h3>
                    <p className="text-3xl font-bold text-jonquil mt-2">{(tasks.filter((task) => task.status == "Pendente" || task.status == "Em Andamento")).length}</p>
                </div>
                <div className="bg-blackOlive p-4 rounded-lg shadow-md border border-amber-400">
                    <h3 className="text-white font-medium">Tarefas Concluídas</h3>
                    <p className="text-3xl font-bold text-jonquil mt-2">{(tasks.filter((task) => task.status == "Concluída")).length}</p>
                </div>
            </div>
